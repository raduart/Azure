...
if($HostNested.HostNestedSupport) {
    ...
    if($hyperv.State -eq "Disabled"){
        # Rafa 20170906 - On next startup of VM will run this script again to create VM on Hyper-v
		Reg add "HKEY_LOCAL_MACHINE\Software\Microsoft\Windows\CurrentVersion\RunOnce" /v ToCreateVM /d "powershell.exe -executionpolicy unrestricted -command C:\Packages\Plugins\Microsoft.Compute.CustomScriptExtension\1.9\Downloads\0\nvm_Add_HyperV_RoleW2k16_VM1.ps1"
		Get-Disk | Where number -eq 2 | Initialize-Disk -PartitionStyle GPT -PassThru | New-Partition -AssignDriveLetter -UseMaximumSize | Format-Volume -FileSystem NTFS -NewFileSystemLabel "Data Disk 1" -Confirm:$false		
    ...
    #Spins up VM
<# Rafael 2017-09-13 Create VM without attaching any VHD. VHD will be attached later.
...
#>
	try {New-VM -Name $VMName -MemoryStartupBytes 3GB -Generation 1 -NoVHD -BootDevice "VHD" -Path "C:\Virtual Machines\$VMName" -ErrorAction SilentlyContinue}
    catch {}
